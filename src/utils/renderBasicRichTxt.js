...
const renderBasicRichTxt = ({ richText, h1, h2, h5, h6, p, a }) => {
  ...
  const H1 = ({ children }) => (
    <h1
      className={`${h1} mt-2 block text-3xl text-center leading-8 font-extrabold tracking-tight text-gray-900 sm:text-4xl`}
    >
      {" "}
      {children}
    </h1>
  ...
  const H6 = ({ children }) => (
    <h6
      className={`${h6} block text-base text-center text-indigo-600 font-semibold tracking-wide uppercase`}
    >
      {" "}
      {children}
    </h6>
